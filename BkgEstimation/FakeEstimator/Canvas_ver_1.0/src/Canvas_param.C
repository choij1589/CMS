...
void Canvas_param(unsigned int histnum, TString xAxisName, TString yAxisName, int yAxisScaleUp, int yAxisScaleDown) {
    //==== Basic Information ====
	...
	vector<TString> histnames
		= {"ElectronEta_", "LeadingJetEta_", "LeadingJetPt_", "corrPt_", // QCD enriched, 0~3
			...
			"LeadingJetPt_", "MET_", "MetPhi_", "Mt_", "corrPt_", 
			"ElectronPhi_", "LeadingJetPhi_", // W enriched, 4~13
			"LeadingElectronEta_", "LeadingElectronPt_", "LeadingJetEta_",
			"LeadingJetPt_", "M(ee)_", "SubLeadingElectronEta_", "SubLeadingElectronPt_",
			"LeadingJetPhi_", "LeadingElectronPhi_", "SubLeadingElectronPhi_"}; // Z enriched 14~23
	...
	if (histnum < 4) histname += Regions.at(0);
	else if (4 <= histnum && histnum < 14) histname += Regions.at(1);
	else if (14 <= histnum && histnum < 24) histname += Regions.at(2);
	else {
    ...
    pUp->Draw();
    pDown->Draw();
	if (trigScale) c->SaveAs(histname + "_ScaleOn.pdf");
	else c->SaveAs(histname + "_ScaleOff.pdf");
}	
