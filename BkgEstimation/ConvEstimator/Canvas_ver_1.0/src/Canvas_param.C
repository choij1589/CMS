...
void Canvas_param(unsigned int histnum, TString xAxisName, TString yAxisName, int yAxisScale, int compScale) {
	//==== Basic information ====
	...
	l->AddEntry(hSyst, "Stat", "f");

	//==== hComp setting ====
	hComp = (TH1D*)hData->Clone("hComp");
	hComp->Divide(hSyst);
	TH1D* hCompSyst = (TH1D*)hComp->Clone("hCompSyst");
