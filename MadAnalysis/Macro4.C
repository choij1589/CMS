...
void Macro4() {
	...
	vector<TH1D*> jet_pt3;
	for (unsigned int i = 0; i < 10; i++) {
		TString histname = "jet_pt_" + to_string(i+1) + "_3_preselection";
		TH1D* h = (TH1D*) files.at(2)->Get(histname);
		jet_pt3.push_back(h);
	}
	jet_pt3.at(0)->GetXaxis()->SetRangeUser(0, 1500);
	jet_pt3.at(0)->GetYaxis()->SetRangeUser(0, 2500);
	...
	cvs->SaveAs("jet_pt_distribution.pdf");
}
