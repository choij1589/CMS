...
public:
	// Constructor
	FileNavi() {
		cout << "2020.06.17 ConvEstimator ver.0.1" << endl;
	...
	void SetElectronIDs(const vector<TString> &ids);

	// Get Functions
	TString GetPathData() const;
	TString GetPathMC() const;
	vector<TString> GetSamples() const;
	vector<TString> GetElectronIDs() const;

	// public functions
	TDirectory* GetDirectory(TString sample, TString dirName) const;
};
